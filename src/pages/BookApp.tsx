...
const BookApp: React.FC = () => {
      ...
      if (idx >= 0) {
        ...
        const walk = (node: Node) => {
          ...
          if (node.nodeType === Node.TEXT_NODE) {
            ...
            for (const child of Array.from(node.childNodes)) { walk(child); if (targetNode) return }
          }
        }
        walk(container)
        if (targetNode) {
          ...
          try {
            ...
            container.scrollTo({ top: container.scrollTop + rect.top - 100, behavior: 'smooth' })
          } catch { container.scrollTo({ top: 0, behavior: 'smooth' }) }
        }
      }
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if ((e.key === 'k' || e.key === 'K') && currentIdx > 0)
        openModal(modalType || 'interp', chapters[currentIdx - 1].name)
      if (e.key === 'b' || e.key === 'B') { if (modalKey) toggleBookmark(modalKey) }
    }
  ...
  const openModal = (type: 'interp' | 'skill' | 'source', key: string) => {
    setModalType(type); setModalKey(key); setToolbarPos(null); setShowPanel(false)
  ...
  const handleNavigate = () => { if (modalBodyRef.current) modalBodyRef.current.scrollTop = 0 }

  const skillDisplayName = modalKey ? skillDisplayNames[modalKey] || modalKey : ''
  const modalTitle = modalType === 'interp' ? `【${modalKey}】原文解读` : modalType === 'skill' ? `【${skillDisplayName}】技能` : `【${modalKey}】原文`
  const rawBody = modalType === 'source'
    ? (sourceContent as Record<string, string>)[modalKey] || '<p style="color:#8080a0;text-align:center;padding:40px 0">未找到该篇原文</p>'
    : modalType === 'interp'
            ...
            <div className="modal-backdrop" onClick={e => { if ((e.target as HTMLElement).classList.contains('modal-backdrop')) closeModal() }}>
              <div className="modal-card">
                <div className="modal-header">
                  {modalType === 'interp' && (
                    <button onClick={() => setTocOpen(v => !v)} title="目录" style={{ background: 'none', border: '1px solid var(--color-border)', borderRadius: 6, padding: '6px', cursor: 'pointer', color: tocOpen ? 'var(--color-gold)' : 'var(--color-text-dim)', display: 'flex', alignItems: 'center', marginRight: 8, transition: 'all 0.2s' }}>
                    ...
                    <div className="action-popover-container">
                      ...
                      {actionPopoverOpen && (
                        <div className="action-popover">
                          <button className="action-popover-item" onClick={() => { setShowPanel(v => !v); setActionPopoverOpen(false) }}><Bookmark size={14} />批注{annotations.length > 0 ? ` (${annotations.length})` : ''}</button>
                          <button className="action-popover-item" onClick={() => { toggleBookmark(modalKey); setActionPopoverOpen(false) }}><Star size={14} fill={isBookmarked(modalKey) ? 'var(--color-gold)' : 'none'} />{isBookmarked(modalKey) ? '已收藏' : '收藏'}</button>
                        </div>
                      )}
                    </div>
                  )}
                  {modalType === 'skill' && modalKey && (
                    <button onClick={handleCopy} style={{ background: 'none', border: '1px solid var(--color-border)', borderRadius: 6, padding: '6px 10px', cursor: 'pointer', color: copied ? 'var(--color-green)' : 'var(--color-text-dim)', display: 'flex', alignItems: 'center', gap: 4, fontSize: 13, transition: 'all 0.2s' }}>
                      <Copy size={14} />{copied ? '已复制' : '复制'}
                    </button>
                  )}
                  <button className="btn-back" onClick={closeModal}><X size={18} /></button>
                </div>
                    ...
                    {modalType === 'interp' && interpToSkill[modalKey]?.length > 0 && (
                      <div className="related-tags">
                        ...
                        {(interpToSkill[modalKey] as string[]).map((sk: string) => (
                          <button key={sk} className="related-tag related-tag-skill" onClick={() => openModal('skill', sk)}>{skillDisplayNames[sk] || sk}</button>
                        ))}
                      </div>
                    )}
