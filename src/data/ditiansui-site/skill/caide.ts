...
<p>在收到八字命局后，通过分析阴阳的天地分布和合化的正邪方向，判断命主的内在品格倾向。此技能适用于人事评估类的命理咨询场景——如合作对象品行判断、用人选才参考、自我修养评估等。但输出仅为命理角度的格局分析，不应作为实际道德评判的唯一依据。</p>
<h2>输入</h2>
<ul>
...
<h2>输出</h2>
<pre><code class="language-typescript">interface CaiDeReport {
  /** 阴阳分布分析 */
  yinYang: {
