...
<p>在收到八字四柱后，先以月令为纲进行初步格局分析：判定月令的当令五行、人元司令（区分日期的用事之神），以及天干（门）与地支司令（地）的配合关系。此技能适用于八字分析的初步阶段——在全面展开用神和格局分析之前，先完成月令维度的核心判断。</p>
<h2>输入</h2>
<ul>
...
<h2>输出</h2>
<pre><code class="language-typescript">interface YueLingReport {
  /** 月令基础信息 */
  monthlyOrder: {
