...
<p>对输入的四柱八字进行假从格分析。当从象判定器输出&quot;疑似从格但不纯粹&quot;时调用此技能。重点检查日主在地支中的微弱残根（余气、墓库）和命局中的杂神干扰，评估从势是否压倒残根，判定假从的类型和危险等级。</p>
<h2>输入</h2>
<ul>
...
<h2>输出</h2>
<pre><code class="language-typescript">interface JiaCongReport {
  /** 命局基础 */
  basic: {
    bazi: string;
    riZhu: string;
