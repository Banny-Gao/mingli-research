...
<p>对输入的四柱八字进行假化格分析。当化格判定器输出&quot;疑似化格但不纯粹&quot;时调用此技能。重点检查化神是否得令、是否有根、是否被冲克，并分析日主原五行属性与化神属性之间的冲突程度，判定假化的类型和危险等级。</p>
<h2>输入</h2>
<ul>
...
<h2>输出</h2>
<pre><code class="language-typescript">interface JiaHuaReport {
  /** 命局基础 */
  basic: {
    bazi: string;
    riZhu: string;
