...
export function runSelfCheckLite(text) {
  ...
  // 检测：「无野诀 / 无自创案例 / 无断章取义 / 全部通过 / 致命错误（X 项）/ ✓/✗ 标记 / 自我评分表 / 自评章节 / 合规分章节」
  // 硬规则扩展（Round 2）：覆盖"**致命错误**：0 项"（无全角括号，仅冒号）/ "## 自评" / "## 合规分" 等章节式复刻
  if (/无断章取义|无野诀|无自创案例|全部通过|致命错误[（:]?\s*\d+\s*项[）:]?|^\s*[-•]\s*(无|✓|✗|已)\s*(断章|野诀|自创|触动|触碰)|深化洞见无|✓\s*$|^##\s*\d*\.?\s*(自评|合规分|致命错误|格式错误|内容检查)/m.test(text)) {
    issues.fatal.push('元自我自评：含"无野诀/全部通过/致命错误（X 项）/自评章节"等 self-check 报告语言')
  }

  // 9. 具体跨篇断言（§一.2 ❌项 5）
  // 检测：「前数篇论 X」「上承/下启/前承/后启」「第 X 篇」「本篇与第 X 章呼应」「后文'论 X'篇当互参」
  // 硬规则扩展（Round 2）：覆盖「前承」/「后启」（之前仅匹配 上承/下启）
  if (/前数篇|(上承|下启|前承|后启).*论|(上承|下启|前承|后启)「|本篇与.*呼应|本篇与.*互参|后文['『].*['』]篇当|前\s*[一二三四五六七八九十\d]+\s*篇|第\s*[一二三四五六七八九十\d]+\s*篇/.test(text)) {
    issues.fatal.push('具体跨篇断言：含"前数篇/上承/下启/前承/后启/第 X 篇"等无前置依据的跨篇关联')
  }
