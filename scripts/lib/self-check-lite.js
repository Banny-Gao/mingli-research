...
export function runSelfCheckLite(text) {
  ...
  // 硬规则扩展（Round 3）：用 positive lookahead（≤4 字符）让「前承」/「后启」+ 章名标记
  //                   （开引号《「【'"」/、/论/章程/格）触发，堵死 LLM 规避 pattern：
  //                   「前承《X》」「前承「X」」「前承X、Y 诸格」「前承论X」
  //                   「前承格局通论之大要」「后启建禄月劫格」等。
  //                   限定 ≤4 字符，避免误伤「上承父母之气」（「上承」+ 名词宾语，
  //                   非跨章引用）这类合法用法。
  if (/前数篇|(上承|下启|前承|后启)(?=.{0,4}[《「【'"」、论章程格])|本篇与.*呼应|本篇与.*互参|后文['『"].*['』"]篇当|前\s*[一二三四五六七八九十\d]+\s*篇|第\s*[一二三四五六七八九十\d]+\s*篇/.test(text)) {
