...
**如何识别要叠加的文字**：从用户原始描述和设计意图中提取**将要以图像形式显示在画面上的具体文字内容**。
- 用户用《》或""显式标记时（如 "《书名》"），直接提取标记内的内容
- 用户用结构化语义描述时（如 "书籍名称：X 作者信息：Y"、"标题：X 副标题：Y"），提取冒号后的实际文字
- 设计意图里 textRequirements 提到的具体文字（如 "《葬书》"或"命理探原"）也要作为视觉文字提取
- 不要提取抽象描述（如 "庄重的标题"、"竖排的副标题"）或修饰词汇
...
每段文字包含以下字段：
- content: 实际显示的文字内容（必填，从用户描述或设计意图中识别提取）— 必填
...
如果没有需要以图像形式显示的具体文字（如背景是纯风景图、或用户完全没有提到任何字面文字内容），输出空数组 []。但请务必先尝试从用户原始描述中识别具体的书名、标题、署名、引文等可叠加文字——只要有就提取，不要因为没看到《》标记就放弃。`
