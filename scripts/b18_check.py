...
files = [
    ...
    'books/渊海子平/articles/合禄格/interpretation.md',
]
...
for fp in files:
    ...
    if lixi:
        ...
        if real_lixi:
            sys.stdout.write(f'  ❌ LLM 输出失控 {len(real_lixi)} 处: {real_lixi[0][:60]}\n')
    ...
    # 8. 元自我断言
    full_match = sum(content.count(p) for p in ['完全一致', '完全相同', '皆不出', '皆同此', '最早定型', '最早期的精炼', '定型表述'])
    if full_match: sys.stdout.write(f'  ⚠️ 元自我断言 {full_match} 处\n')

    # 9. 元数据块
    meta_pattern = re.findall(r'>\s*\*\*(?:本篇模式|模式判定|体量定位|底本|体裁|写作模式|本篇性质|典籍定位|写作原则|原文出处|所属典籍|原文体例|原文体量|定位|本题位置|位置)\*\*', content)
