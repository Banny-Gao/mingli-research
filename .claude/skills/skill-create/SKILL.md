...
- **路径必须在 CATEGORY_TREE 注册**（副本：`shared/sources/category-tree.json`；录入自 `scripts/lib/category-tree.js`）
...
- 不做批量模式
- 不做干跑预览门点
- 不直接调 `scripts/generate.js`（CI 自动触发，git commit 后跑 `npm run generate`）
...
| 类别 | 触发 | 处置 |
|------|------|------|
| 1. CATEGORY_TREE 未注册 | 所选类别路径未在 `shared/sources/category-tree.json` 注册 | 阻断，2 选项门：先注册类别（改 `scripts/lib/category-tree.js` 后重跑录入）/ 退出 |
