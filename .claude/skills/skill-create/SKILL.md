...
- **严格遵守** `research-dispute/SPEC-skill.md`（v2 类别化版）+ `research-dispute/general.md` + 术数专项（如 `bazi.md`）
- **产物**：`skills/{一级类别}/{二级类别}/{skill-slug}/SKILL.md` + `rules/<书slug>.md`
- **路径必须在 CATEGORY_TREE 注册**（真源：`scripts/lib/category-tree.js`）
...
- [ ] 读过 `research-dispute/SPEC-skill.md`（v2 类别化版）
