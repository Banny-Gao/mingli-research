...
**任一副本缺失立即终止**。恢复方式：跑 `node scripts/ingest-skill-sources.js --target book-create` 重新录入。

## 指纹校验（动态化）

**不存死值。** book-create 与 self-check 共享同一份规范文件，规范本体的任何修改都会让死值漂移。本节改用实时校验：主 agent 在 Step 3 启动时跑 `scripts/self-check-fingerprint.py --project-root shared/sources --source-prefix ""`，**只取 2 个相关副本**的指纹。
