...
## 严重度分级（与 report-template 对齐）

- **fatal** — 必须用户确认才能继续的内容缺陷（空白文件、缺目录结构、必含规范未引、报告骨架缺失等）。≥1 即触发主 agent 落盘 gate。
- **error** — 单条规则违反，需修订但不阻塞流程；建议下一轮自检前修。
- **warn** — 风格 / 可读性 / 幂等性建议，不影响合规判定。
- **info** — 提示性观察（同步状态、统计口径差异等）。

子 SKILL.md 派发 subagent 时必须按上表打级，主 agent 在 Step 6 路由时按 `fatal ≥ 1` 判断是否走落盘 gate。
