...
- **字形策略**（可选，默认 `原文照录`）声明该书 `source.md` 的字形处理策略：
  - `原文照录`：保留底本原字形（含异体、繁体、避讳等），不规范化
  - `简体规范化`：将底本异体/繁体统一为简体（如《正统道藏》本化简）
  > 写入 blockquote 后即被视作正式元信息；不写则视为默认 `原文照录`
