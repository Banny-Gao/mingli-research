...
> **用途：** 规范典籍目录结构与生成规则
> **版本：** 3.1 — 去掉路径列、新增人工确认 gate、slug 支持中文
...
**规则：**
- `##` 二级标题表示分类（如"上篇 · 通神论"）
- 表格 4 列：编号、篇名、状态、关联技能
- 原文路径和解读路径不再写入表格——ChapterKey 统一后，路径由篇名推导：
  - 原文：`articles/{篇名}/source.md`
  - 解读：`articles/{篇名}/interpretation.md`
  - 技能：`articles/{篇名}/skill.md`
- 状态：`已解读` | `待解读`
- 关联技能：使用 **displayName**（中文技能名），多个用逗号分隔，无技能则为空
- 关联技能的 displayName 必须与对应 `articles/{篇名}/skill.md` 中 YAML frontmatter 的 `displayName` 严格一致
...
skill.md 的 YAML frontmatter 和内容结构详见 **SPEC-skill.md**。catalog.md 仅通过 `displayName` 与之关联。
...
3. 术数取值：`山` | `医` | `命` | `相` | `卜`
4. 遍历 `##` 标题为篇章分类名
5. 解析表格（4 列）：编号、篇名、状态、关联技能（displayName）
...
- [ ] 术数取值为 `山` | `医` | `命` | `相` | `卜`
- [ ] 表格 4 列（编号、篇名、状态、关联技能），路径由篇名推导
- [ ] 关联技能使用 displayName，多个用逗号分隔
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v3.1 | 2026-05-25 | 去掉原文/解读路径列（ChapterKey 统一后由篇名推导）；新增人工确认 gate；slug 支持中文 |
| v3.0 | 2026-05-24 | 通用化：支持山医命相卜所有典籍；术数/内容类型字段标准化 |
| v2.0 | — | 引入 section 分类，关联结构 |
| v1.0 | — | 初始版本，仅滴天髓 |
