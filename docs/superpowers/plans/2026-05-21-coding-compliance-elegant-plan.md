...
].filter(({ data }) => data?.length).map(({ key, data, label }) => (
  <div key={key} className="related-tags">
    <span className="related-label">{label}</span>
    {(data as string[]).map(sk => <button key={sk} onClick={() => onNavigate(key, sk)}>...</button>)}
  </div>
))}
```
...
```tsx
// 反模式：状态判断散落
if (chapter.isDone) { /* 在读 */ }
if (!chapter.isDone && hasAnnotations) { /* 已批注 */ }
...
const checkFrame = (currentTime: number) => {
  if (currentTime - lastTime > 20) { // > 2 frames @ 60fps
...
```json
{
  "performance": {
    ...
    "long_tasks": [
      { "duration": 85, "threshold": 50, "file": "src/hooks/useSearch.ts" }
    ],
    "frame_drops": [
      { "drop_ms": 45, "file": "src/components/ModalReader.tsx", "line": 120 }
    ]
  }
}
```
...
// 优雅：配置选项
const useBookmark = (slug: string, options?: {
  autoSync?: boolean
